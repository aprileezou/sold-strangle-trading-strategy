...
# Initilize port_ID before each run
port_ID=''
...
for (i in 1:length(Main$date)){
  if (Main$date[i] %in% first_business_days){
    ...
    if (dim(position)[1] == 0){
      ...
      Main$short_signal[i] = TRUE
      
      # port_ID is initialized to avoid problems caused by legacy port_ID
      port_ID = ''
      if (Main$iv_put[i]<16){
        next
      }
    ...
    if (k <= length(Main$date)){
      ...
      if (FALSE && (Main$long_indicator[k] > 1.02) && (Main$long_indicator[k-1] > 1.02) && (Main$date[k]<expiry_date) && port_ID != "") {
        #print("not natural")
