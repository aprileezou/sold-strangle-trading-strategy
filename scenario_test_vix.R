...
library("xlsx")
library("zoo")
vix <- read_excel("VIX.xlsx",col_names=c('weekday','date','vix'))
vix = vix[!is.na(vix$vix),c('date','vix')]
vix$date=as.Date(vix$date)

# There is no missing value in vix and type is already numeric.
Main <- merge(Main,vix,by='date',all.x = TRUE)
...
for (i in 1:length(Main$date)){
  if (Main$date[i] %in% first_business_days){
    ...
    ## Change here when time horizon changes
    k = i + 1
    if (k <= length(Main$date)){
      Main$long_indicator[k] = Main$last_spx[k]/Main$last_spx[k-1]
      ## compares vix here instead of long_indicator/returns
      if ( (Main$vix[k]<11) && (Main$date[k]<expiry_date) && port_ID != "") {
        #print("not natural")
